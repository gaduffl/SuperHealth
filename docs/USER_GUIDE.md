...
- **Add to list** in a biomarker's own action menu puts it on any of your biomarker lists: the lists are ticked, not the marker, and the ones it is already on start ticked. The retest interval in that dialog applies only where it is newly added, so a schedule you set for a marker on a particular list is never overwritten by it; leave the field empty for a membership without due alerts. A new list can be created from inside the dialog. Unticking a list takes the marker off that list and leaves every other one alone.
