...
A plan exports two different documents. The PDF, CSV, and JSON formats carry the whole plan — every tier, the reasoning, the independent review, and what each cheaper tier gives up — for the person who made it. `buildTierRequest` carries one tier's ticked tests and nothing else, because a consultation asks only which tests are being requested; it keeps a single line of provenance so a printed list is not mistaken for a requisition. `LabPlan.selectedItemsThrough` is the one place the ticks are read, so the screen, the tier picker, and the document cannot disagree about what was chosen.

A biomarker list is reachable from both ends. The lists sheet edits one list's membership; `setBiomarkerListMemberships` edits one marker's, taking the set of lists it should be on and writing only the difference. An untouched membership keeps its own row — interval, notes and `updated_at` — because ticking an already-ticked box is not a request to reset a per-marker retest schedule, and a no-op edit should not manufacture sync work on every device.

The doctor's PDF (`lib/export/biomarker_report_export_service.dart`) is assembled in two pure steps before any drawing: `buildBiomarkerReportRequest` groups included biomarkers by dashboard category and takes each chart's points, unit and band from the shared `biomarkerTrendData`; `annotateBiomarkerExport` letters the commented reports that fed an included chart in date order and numbers each chart's reading notes by day. The PDF only draws those results, so the linking rules are tested without rendering.

A biomarker list is a retest schedule. `biomarker_lists.due_interval_days` is the list's interval; an item's own `due_interval_days` overrides it, and an item without one follows the list (`BiomarkerList.intervalFor`). Only when neither is set is the item a plain checklist entry that never becomes due. Before v14 the list had no interval, so an item added without one — every member of a package, for instance — was silently never due and therefore never mandatory for the lab planner. The v14 migration gives each list the most common interval among its items (the shorter on a tie) and clears the item copies that merely repeat it. The legacy import keeps the old app's list `due_duration` on the list the same way, instead of copying it into every item. A lab plan is a snapshot, so the plan view names any currently due marker the plan does not contain.

Biomarker management is a hub of sections rather than one scroll: `_BiomarkerWorkspaceScreen` renders either the hub or a single section, so a status deep link from a Today tile opens that section directly and nothing else has to be scrolled past. `LabReportScreen` reads a document by ID from the controller rather than holding it, so a re-parse, an edit, or a sync arriving while it is open is reflected instead of frozen; it is the one place a parsed value can be judged beside the rest of its own extraction.

`LabPlanPricing` decides which catalog packages to apply, and the tier renders each applied one as a line item beside the tests: a tri-state checkbox over the planned tests it covers, its price and saving, and its full membership. Selecting a package writes every covered test in one plan save, and rows already in the wanted state keep their `updated_at` so an ordinary selection does not manufacture sync work.
